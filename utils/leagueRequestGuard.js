...
async function guard(path, options, read) {
  ...
  if (path.startsWith('rpc/')) {
    if (['rpc/configure_cup_byes', 'rpc/append_configured_tournament_fixture', 'rpc/append_tournament_fixture', 'rpc/bot_replace_disabled_club', 'rpc/bot_delete_tournament'].includes(path)) {
      await assertTournament(['rpc/bot_replace_disabled_club', 'rpc/bot_delete_tournament'].includes(path) ? body?.p_id : body?.p_tournament);
      if (path === 'rpc/bot_replace_disabled_club' && body?.p_guild !== guild) deny();
      if (path === 'rpc/bot_delete_tournament' && body?.p_tipo !== league) deny();
