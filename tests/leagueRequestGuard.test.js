...
test('foreign fixtures, enrollments, RPC reports, and players rejected by ID', async () => {
 await scope.run({guildId:ash},async()=>{
  ...
  await request('rpc/bot_delete_tournament',{method:'POST',body:{p_id:'a',p_tipo:'ash'}});
  await assert.rejects(request('rpc/bot_delete_tournament',{method:'POST',body:{p_id:'t',p_tipo:'ash'}}),/otra liga/);
  await assert.rejects(request('rpc/bot_delete_tournament',{method:'POST',body:{p_id:'a',p_tipo:'tematico'}}),/otra liga/);
